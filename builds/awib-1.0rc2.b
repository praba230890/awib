[
Also Written In Brainfuck (awib) v1.0-rc2

A brainfuck compiler written in brainfuck for Linux on i386 by Mats Linander

This program takes brainfuck source code as input, compiles it into machine
code and outputs it as an elf binary. See the README for additional info.
...
Awib will only run properly in brainfuck environments where:
 ...
 (*) executing a ',' (read) after end of input stores EOF in the cell
     at the pointer
 (*) EOF = -1 (0xff) (awib can easily be modified to accept EOF = 0)
